...
// and the balance on its foot -- a new account's 30 of 50, which is
// exactly the day's refill (core/credits.py seeds it on first read).
...
// it has one, and the day's refill -- what a fresh account holds --
// until then. Same classes as components/credits/BalanceLine.jsx, so
...
function PrintedBalance() {
  ...
  const balance = credits?.unlimited ? null : (credits?.balance ?? DAILY_REFILL)
  return (
