...
def _card_stats(states: dict, user_id: str, raw_id: str, mode: str) -> dict:
    """Full per-card SRS stats for the detail panel, not just a status label."""
    ...
    for word in llm_result.get("words", []):
        ...
        if vocab_match:
            ...
            word_entry["vocab_match"] = {
                "level": level,
                "raw_id": raw_id,
                "entry": _serializable_entry(entry),
                "stats": _card_stats(states, user_id, raw_id, VOCAB_STATUS_MODE),
            }
        ...
        for char, level, entry, raw_id in kanji_matches:
            word_entry["kanji_matches"].append({
                ...
                "raw_id": raw_id,
                "entry": _serializable_entry(entry),
                "stats": _card_stats(states, user_id, raw_id, KANJI_STATUS_MODE),
            })
