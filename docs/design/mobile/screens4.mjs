...
// cards, the rank, the seal). One movement per screen, CSS only.
import { I, pass, jourLine } from './parts.mjs'
...
const IC = {
  ...
  bell:    svg('<path d="M6 16V11a6 6 0 0 1 12 0v5l2 2H4z"></path><path d="M10 20a2 2 0 0 0 4 0"></path>'),
}

// ── the frame: back · the track · the count ──
const STEPS = 8
const head = (step) => `<div class="brd__head">
  <button type="button" class="brd__back">${I.chevL}</button>
  <div class="brd__track"><div class="brd__done" style="width: ${Math.round(100 * step / STEPS)}%;"></div><span class="brd__train" style="left: ${Math.round(100 * step / STEPS)}%;"></span></div>
...
const opt = ({ on = false, icon = '', code = '', label, desc = '', jp = '', tag = '' }) => `<button type="button" class="brd-opt${on ? ' brd-opt--on' : ''}">
  ${icon ? `<span class="brd-opt__icon">${icon}</span>` : ''}${code ? `<span class="brd-opt__code">${code}</span>` : ''}
  <span class="brd-opt__names"><span class="brd-opt__label">${label}</span>${desc ? `<span class="brd-opt__desc">${desc}</span>` : ''}</span>
  ${tag ? `<span class="brd-tag">${tag}</span>` : ''}${jp ? `<span class="brd-opt__jp" lang="ja">${jp}</span>` : ''}
  <span class="brd-opt__check">${I.check}</span>
</button>`

// ── 1 · welcome: the sign, the rolling stock, one door ──
const demo = (color, tag, inner, meaning, foot) => `<div class="brd-demo" style="--line-color: ${color};">
  <span class="brd-demo__tag">${tag}</span>${inner}
  <span class="brd-demo__meaning">${meaning}</span><span class="brd-demo__foot">${foot}</span>
</div>`
const DEMOS = [
  demo('var(--line-kanji)',   'Kanji',      '<span class="brd-demo__glyph" lang="ja">駅</span>', 'station', 'Kanji → meaning'),
  demo('var(--line-vocab)',   'Vocabulary', '<span class="brd-demo__glyph" lang="ja">食べる</span>', 'to eat', 'Word → meaning'),
  demo('var(--line-grammar)', 'Grammar',    '<span class="brd-demo__glyph brd-demo__glyph--sm" lang="ja">雨が降り<span class="cloze">そう</span>です。</span>', 'Fill in', 'Rule → sentence'),
  demo('var(--line-kanji)',   'Kanji',      '<span class="brd-demo__draw"><svg viewBox="0 0 100 100"><path d="M30 28h40M50 28v44M32 72h36"></path></svg></span>', 'Draw it', 'Meaning → kanji'),
  demo('var(--line-reading)', 'Listening',  '<span class="brd-demo__wave"><i></i><i></i><i></i><i></i><i></i><i></i></span>', 'Listen', 'Sound → meaning'),
  demo('var(--line-reading)', 'Reading',    '<span class="brd-demo__glyph brd-demo__glyph--sm" lang="ja">駅で友達を<br>待っています。</span>', 'Read it', 'Sentence → meaning'),
].join('')
const DEMOS2 = [
  demo('var(--line-vocab)',   'Vocabulary', '<span class="brd-demo__glyph" lang="ja">切符</span>', 'ticket', 'Word → meaning'),
  demo('var(--line-kanji)',   'Kanji',      '<span class="brd-demo__glyph" lang="ja">山</span>', 'mountain', 'Kanji → reading'),
  demo('var(--line-reading)', 'Reading',    '<span class="brd-demo__glyph brd-demo__glyph--sm" lang="ja">明日は雨が<br>降ると思う。</span>', 'Read it', 'Sentence → meaning'),
  demo('var(--line-grammar)', 'Grammar',    '<span class="brd-demo__glyph brd-demo__glyph--sm" lang="ja">駅まで<span class="cloze">歩いて</span><br>行きます。</span>', 'Fill in', 'Rule → sentence'),
  demo('var(--line-kana)',    'Kana',       '<span class="brd-demo__glyph" lang="ja">きっぷ</span>', 'ki · p · pu', 'Kana → sound'),
  demo('var(--line-exam)',    'Mock exam',  '<span class="brd-demo__glyph brd-demo__glyph--sm">Part 3 · Q7<br><span lang="ja">毎朝、駅まで＿＿</span></span>', '24:18', 'Timed paper'),
].join('')
const WELCOME = `
...
// ── 2 · the name: the field, the caret, Continue right under it ──
const NAME = `
<main class="brd">
  ${head(1)}
  ${q("What's your name?")}
  <div class="brd-field">Aiko<span class="brd-field__caret"></span></div>
  ${depart('Continue')}
</main>`

// ── 3 · why: five doors, one picked ──
const WHY = `
<main class="brd">
  ${head(2)}
...
// ── 4 · the kana check: two panes, four honest answers, no Continue ──
...
<main class="brd">
  ...
  ${kanaCard()}
  <div class="brd__opts">
    ${opt({ label: 'Hiragana', jp: 'すし' })}
    ${opt({ label: 'Katakana', jp: 'ホテル' })}
    ${opt({ label: 'Both' })}
    ${opt({ label: 'Not yet' })}
  </div>
</main>`
...
<main class="brd">
  ...
  ${kanaCard(true)}
  <p class="brd__hint">Two scripts, 46 signs each. Your first stop.</p>
  ${foot(depart('Continue'))}
</main>`

// ── 5b · the level: six stops, one line each ──
const LEVELS = [
...
<main class="brd">
  ...
  <div class="brd__opts">
    ${opt({ code: 'N4', label: 'Intermediate', desc: 'Everyday talk', tag: 'Next stop', on: true })}
    ${opt({ code: 'N3', label: 'Advanced',     desc: 'Daily life with ease' })}
    ${opt({ code: 'N2', label: 'Expert',       desc: 'News, work, novels' })}
    ${opt({ code: 'N1', label: 'Fluent',       desc: 'Anything, anywhere' })}
  </div>
  ${foot(depart('Continue'))}
</main>`

// ── 7 · the rhythm: four cards, one recommended ──
const minCell = (n, on = false, tag = '') => `<button type="button" class="brd-cell${on ? ' brd-cell--on' : ''}">
...
const hourCell = (label, time, on = false) => `<button type="button" class="brd-cell brd-cell--sm${on ? ' brd-cell--on' : ''}"><span class="brd-cell__label">${label}</span><span class="brd-cell__time">${time}</span></button>`
const TIME = `
<main class="brd">
  ${head(7)}
  ${q('When do you study?')}
  <div class="brd-board">
    ...
    <div class="brd-board__flaps">${flap(0)}${flap(7)}<span class="brd-board__colon">:</span>${flap(3)}${flap(0, true)}</div>
  </div>
  ...
  <div class="brd-day">
    ...
    <span class="brd-day__tick" style="left: 5%;">06</span><span class="brd-day__tick" style="left: 37%;">12</span><span class="brd-day__tick" style="left: 68%;">18</span><span class="brd-day__tick" style="left: 100%;">24</span>
    <button type="button" class="brd-day__train" style="left: 13%;"></button>
  </div>
  ${foot(depart('Continue'))}
</main>`
...
const NOTIF_BODY = `
  ...
  <div class="brd-notif">
    ...
    <div class="brd-notif__body">
      ...
      <span class="brd-notif__text">8 cards are waiting at the gate.</span>
    </div>
  </div>
...
<div class="brd-dim">
  <div class="brd-alert">
    <div class="brd-alert__body"><span class="brd-alert__title">“Japanese Learner” would like to send you notifications</span><span class="brd-alert__text">One reminder a day, at 07:30.</span></div>
...
// ── 10 · building the journey: the train runs the stops ──
const stop = (state, label, val = '') => `<div class="brd-stop brd-stop--${state}">
  <span class="brd-stop__label">${label}</span>
  ${state === 'done' ? `<span class="brd-stop__check">${I.check}</span>` : state === 'now' ? '<span class="brd-stop__dots"><i></i><i></i><i></i></span>' : ''}
  ${val ? `<span class="brd-stop__val">${val}</span>` : ''}
</div>`
const BUILDING = `
<main class="brd">
  <div class="brd-build">
    ...
    <div class="brd-stops">
      <div class="brd-stops__rail"></div><div class="brd-stops__done"></div><span class="brd-stops__train"></span>
      ${stop('done', 'Your goal', 'N5 → N4')}
      ${stop('done', 'Your lines', 'Four lines')}
      ${stop('now',  'Your daily ride', '10 min · 07:30')}
      ${stop('next', 'Your projection')}
    </div>
  </div>
</main>`

// ── 11 · the plan: the projection, then the promise ──
// 326 × 150 plot; y = words remembered, x = Sep 2026 → Mar 2027.
...
const CHART = `<div class="brd-chart">
  <span class="brd-chart__title">Words you'll remember</span>
  <svg viewBox="0 0 326 150">
    ...
    <text class="lbl" x="252" y="24" text-anchor="end">~1,500 · daily reviews</text>
    <text class="lbl lbl--soft" x="298" y="118" text-anchor="end">~440 · cramming</text>
  </svg>
...
<main class="brd">
  ...
  ${CHART}
  <p class="brd-lead">With <b>10 min a day</b>, by <b>March 2027</b>:</p>
  <div class="brd-bullets">
    <div class="brd-bullet">${I.check}~1,500 words</div>
    <div class="brd-bullet">${I.check}~300 kanji</div>
    <div class="brd-bullet">${I.check}Everyday conversations</div>
    <div class="brd-bullet">${I.check}Ready for JLPT N4</div>
  </div>
  ${foot(depart('Continue'))}
</main>`

// ── 12 · the welcome offer: the pass, the perks, two plans, one action ──
const plan = (label, price, on = false, tag = '') => `<button type="button" class="brd-plan${on ? ' brd-plan--on' : ''}">
...
<main class="brd" style="gap: var(--sp-4);">
  <div class="brd-offer">
    ...
    <span class="brd-offer__sub">on the yearly pass, today only</span>
  </div>
  <div class="brd-perks">
    ...
    <div class="brd-perks__list">
      <span class="brd-perk">${I.check}Unlimited reviews</span>
      <span class="brd-perk">${I.check}All four lines, your decks</span>
      <span class="brd-perk">${I.check}Mock exams, analyzer</span>
    </div>
  </div>
  ...
  ${foot(depart('Get −X%'), 'Continue with the free pass', 'Cancel anytime.')}
</main>`
...
<main class="brd">
  ...
  <p class="brd__hint" style="text-align: center; margin-top: calc(-1 * var(--sp-3));">Enjoy the ride.</p>
  <div class="brd-issue">
    ${pass({ name: 'Aiko', level: 1, into: 0, span: 100, rankJp: '浪人', rank: 'Rōnin', footer: jourLine('onTime') + balanceLine() }).replace('<h1 class="pass__brand-sub">', '<span class="pass__brand-sub">').replace('</h1>', '</span>').replace(/<button type="button" class="jour-line__turn">[^<]*<\/button>/, '')}
    <span class="brd-issue__seal" lang="ja">発行</span>
  </div>
...
<div class="mo">
  <div class="mo__row">
    <div class="mo__col mo__col--wide">
      ...
      <p class="spec__note">Fast rhythm, never rushed: a screen's own movement starts after the pull lands (<b>+120 ms</b>) and is over within <b>500 ms</b>, so the learner is never waiting for a decoration.</p>
    </div>
    <div class="mo__col">
      <span class="spec__label">One movement per screen</span>
      <div class="mo__list">
        ${item('Welcome', 'the cards roll past like a train, 26 s a lap; the drawing stroke redraws, the sound bars breathe')}
        ${item('Name', 'the field is already focused, the caret blinks')}
        ...
        ${item('Rhythm · Hour', 'the picked card lifts to its wash; the board flips its last digit')}
        ${item('Nudge', 'the notification drops in from the top and settles')}
        ${item('Building', 'the train rides the stops, each one lights and ticks')}
        ${item('Plan', 'the two lines draw themselves, yours last; the promises stagger in')}
        ${item('Offer', 'the figure stamps in')}
  ...
  <div class="mo__row">
    <div class="mo__col mo__col--wide">
      ...
      <div class="mo__list">
        ${item('Continue is never hidden', 'it sits above the keyboard inset; on the name step it lives right under the field, so the keyboard cannot reach it')}
        ${item('The language is the device\'s', 'read from the locale, never asked')}
        ${item('One filled action', 'gold, 52 px, full width; disabled is opacity 0.45 and nothing else')}
        ${item('Back is always there', 'the answers are kept; a return never costs anything')}
        ${item('Optimistic, never lying', 'every figure comes from the learner\'s own answers and wears a ~; the chart names its terms')}
        ${item('The theme in small doses', 'the track, the board, the pass, the seal — no announcements, no jargon before the station')}
        ${item('Explicit', 'every choice shows what it is: the kana options carry their sample, the levels their line, the plans their price')}
        ${item('Still frames read', 'every movement rests on a state that stands on its own — reduced motion loses nothing')}
      </div>
    </div>
    <div class="mo__col">
      <span class="spec__label">Branches and exits</span>
      <div class="mo__list">
        ...
        ${item('Offer → free pass', 'the ghost link boards without paying and keeps the offer from returning')}
