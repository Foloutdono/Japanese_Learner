...
| `.stroke-pad*` (`__key`, `__key--on`), `.radical-page*` (`__grid`, `__grid--labelled`, `[data-fill="short"]`), `.radical-tile*` (`__sub`, `__run`, `--started`), `.dict-mark*` | the radical index as a study source (plan 086) — the dictionary's own, read under the station's `--line-color`, a tile printing the meaning and `learned / total` over the course's kanji. Every stroke count is a key on a pad that wraps (it was a rail that scrolled sideways), and one page is one grid in one order | `components/dictionary/RadicalIndex.jsx`, `components/selection/RadicalSelector.jsx` |
