...
| `.console` + `.chip` (a chip per collection in its line colour, the radical index as a fifth chip under kanji), `.console__index` with the count (the dots while it loads) | the console | `DictionaryScreen.jsx` on `components/chrome/Console.jsx` |
...
| `.bar` + `.stage__leave`, `.rep-card` (`--rows`), `.rep-head`, `.rep-fig` (`--title`, `__u`), `.rep-delta`, `.rep-cap` | the statistics head and its inserts (plan 085) — the bar wears TO, the plate the profile's door to it already draws, in the pass ink that door is painted in; no block under it carries a heading | `screens/StatsScreen.jsx` |
| `.rep-line` (`__svg`, `__path`, `__lone`, `__now`), `.rep-axis` | the retention line — twelve weeks, good-or-better over reviews, a gap where a week had nothing, the latest week pressed in the stamp's lacquer | `components/stats/RetentionLine.jsx` |
| `.rep-ladder` (`__bar`, `__rung`, `__caps`, `__cap`) | the strength ladder — five rungs as wide as their share, the mastered ink deepening rightward | `components/stats/StrengthLadder.jsx` |
| `.rep-lines` (`--sheet`), `.rep-line-row` (`--level`, `__name`, `__pct` `--none`), `.composition` (`__seg` `--mastered`/`--learning`), `.pf-line__roundel` | by line — the ledger's roundel, the composition in the state inks, the retention; a row opens a `Sheet` per level | `components/stats/LineRows.jsx` |
| `.trouble` (`--sheet`, `__row` `--open`, `__glyph`, `__meta`, `__where`, `__accuracy-track`, `__accuracy-fill`, `__accuracy-value`, `__lapses`, `__go`, `__more`) | the six weakest cards under the misses figure, the twelve in a `Sheet` | `components/stats/TroubleList.jsx` |
...
| `.stroke-rail*` (`__track`, `__key`, `__key--on`, `__step`), `.radical-page*` (`__grid`, `__grid--labelled`, `[data-fill="short"]`), `.radical-tile*` (`__sub`, `__run`, `--started`), `.dict-mark*` | the radical index as a study source (plan 086) — the dictionary's own, read under the station's `--line-color`, a tile printing the meaning and `learned / total` over the course's kanji. The stroke counts are one line walked by two chevrons (the track is `overflow: hidden`: scrolled by the steps, never dragged), and one page is one grid in one order | `components/dictionary/RadicalIndex.jsx`, `components/selection/RadicalSelector.jsx` |
