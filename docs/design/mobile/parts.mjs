// Shared furniture: the chrome (HUD + tab bar), the compact header, icons,
// the pass, the stamp rally, the wall-map track. Every artboard composes
// these so the same object is drawn the same way on every screen.
//
// The mobile ruling, decided with the owner (2026-09-05): the interface
// speaks the learner's language; Japanese is content — a word, a kanji, a
// sentence, a deck's name, a rank's glyph — never a label. The tab bar's
...
export const I = {
  ...
  image: path('<rect x="3" y="4" width="18" height="16" rx="2"></rect><circle cx="9" cy="10" r="2"></circle><path d="M21 16l-5-5-8 8"></path>'),
  ...
  more: path('<circle cx="5" cy="12" r="1.6"></circle><circle cx="12" cy="12" r="1.6"></circle><circle cx="19" cy="12" r="1.6"></circle>'),
  flag: path('<path d="M5 21V4h12l-2 4 2 4H5"></path>'),
  books: path('<path d="M4 5h6v14H4zM10 5h6v14h-6zM16 7l4-1v13l-4 1z"></path>'),
}

// ── the HUD: level · station panel · commuter pass ──
...
export function hud({ level = 12, st = 'onTime', cr = 30, gain = null } = {}) {
  ...
  <button type="button" class="hud__level${gain ? ' hud__level--gain' : ''}"><span>${level}</span>${gain ? `<span class="hud-fare">+${gain}<span class="hud-fare__unit">xp</span></span>` : ''}</button>
  ${status(st)}
  ${credits(cr)}
</div></div>`
}

// ── the tab bar: five gates. The kanji is the icon; the caption is the label. ──
const TABS = [
...
export function tabbar(active = 'today', due = 24) {
  return `<nav class="tabbar">${TABS.map(t => {
    const badged = t.id === 'today' && due
    return `
  <button type="button" class="tab${t.id === active ? ' tab--on' : ''}${badged ? ' tab--badged' : ''}">
    <span class="tab__jp" lang="ja">${t.jp}</span>${badged ? `<span class="tab__due">${due}</span>` : ''}
    <span class="tab__cap">${t.cap}</span>
  </button>`
  }).join('')}
</nav>`
}

// ── the compact header: roundel · title · sub … aside, a pigment rule under it ──
export function header({ code = '', title, sub = '', color = 'var(--accent2)', aside = '', register = false }) {
  return `<div class="bar${register ? ' bar--register' : ''}" style="--line-color: ${color}">
  <div class="bar__row">
    ${code ? `<span class="bar__roundel">${code}</span>` : ''}
    <span class="bar__names"><h1 class="bar__title">${title}</h1>${sub ? `<span class="bar__sub">${sub}</span>` : ''}</span>
    ${aside ? `<span class="bar__aside">${aside}</span>` : ''}
  </div>
  <div class="bar__stripe"></div>
</div>`
}
export const clock = (hh = '08', mm = '14') => `<span class="board-clock">${hh}<span class="board-clock__colon">:</span>${mm}</span>`
export const back = (label) => `<button type="button" class="stage__leave">${I.chevL}<span>${label}</span></button>`
...
export const due = n => n ? `<span class="wmap-due">${n}<span class="wmap-due__unit">due</span></span>` : ''

// ── the stamp rally: the last seven days, today pressed ──
const RALLY = [
  { d: 'S', on: true,  tilt: -2 }, { d: 'M', on: true, tilt: 4 }, { d: 'T', on: true, tilt: -5 },
  { d: 'W', on: false, tilt: 0 },  { d: 'T', on: true, tilt: 3 }, { d: 'F', on: true, tilt: -3 },
  { d: 'S', on: true,  tilt: 2, today: true },
]
export function rally(streak = 3) {
  return `<div class="stamp-rally">
  <span class="stamp-rally__row">${RALLY.map(s => `<span class="stamp-rally__stamp${s.on ? '' : ' stamp-rally__stamp--missed'}${s.today ? ' stamp-rally__stamp--today' : ''}" style="--stamp-tilt: ${s.tilt}deg">${s.d}</span>`).join('')}</span>
  <span class="stamp-rally__label"><span class="stamp-rally__count">${streak}<span class="stamp-rally__unit">days</span></span><span class="stamp-rally__caption">Streak</span></span>
</div>`
}
export function pace(n = 4, target = 10) {
  return `<span class="hall-pace">
  <span class="hall-pace__latin">New items</span>
...
export function jourLine(st = 'onTime', valid = '14 Mar 2027') {
  ...
  <span class="jour-line__status"><b>${p.word}${p.delta ? ` · ${p.delta}` : ''}</b></span>
  <span class="jour-line__validity"><span class="jour-cap">Valid until</span><b>${valid}</b></span>
  <button type="button" class="jour-line__turn">Turn over ↻</button>
</div>`
}
